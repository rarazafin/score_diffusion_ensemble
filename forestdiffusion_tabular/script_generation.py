...
parser.add_argument('--agg', type=str, default='arithmetic', help='aggregation scheme for random forest')
parser.add_argument('--fixed_tree_index', type=int, default=0, help='')
        ...
        for method_index in range(method_index_start, len(args.methods)):
            ...
            for n in range(args.nexp):
                ...
                if method == 'oracle':
                    ...
                    print("Training forest diffusion")
                    if args.ycond and (bin_y or cat_y):
                        forest_model = ForestDiffusionModel(X=Xy_train_used[:,:-1], 
                            ...
                            seed=n,
                            agg=args.agg,
                            fixed_tree_index=args.fixed_tree_index)
                    else:
                ...
                for gen_i in range(args.ngen):
                    ...
                    if Xy_train.shape[0] < OTLIM:
                        score_W1_train[method] += pot.emd2(pot.unif(Xy_train_scaled.shape[0]), pot.unif(Xy_fake_scaled.shape[0]), M = pot.dist(Xy_train_scaled, Xy_fake_scaled, metric='cityblock'),numItermax=1000000) / (args.nexp*args.ngen)
                        score_W1_test[method] += pot.emd2(pot.unif(Xy_test_scaled.shape[0]), pot.unif(Xy_fake_scaled.shape[0]), M = pot.dist(Xy_test_scaled, Xy_fake_scaled, metric='cityblock'),numItermax=1000000) / (args.nexp*args.ngen)
            ...
            if method == 'forest_diffusion':
                ...
                if args.forest_model == 'xgboost':
                    ...
                    method_str += f"depth={args.max_depth} n_trees={args.n_estimators} aggregation={args.agg} "
            ...
            if args.add_missing_data:
                ...
                print("W1:", score_W1_train[method], score_W1_test[method])
            ...
            csv_str += f"\n"
            print(csv_str)
            path = f"results_{dataset}_{args.agg}_{args.n_estimators}.txt"
            with open(path, 'a+') as f: # where we keep track of the results
                f.write(csv_str)

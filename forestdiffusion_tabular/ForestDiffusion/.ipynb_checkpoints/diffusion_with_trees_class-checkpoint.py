...
class ForestDiffusionModel():
  ...
  def predict_n(self, f, X_test):
    ...
    if self.agg is None or self.agg == "arithmetic":
        return np.mean(out, axis=0)
    elif self.agg == "geometric":
        return np.exp(np.mean(np.log(out + 1e-9), axis=0))
    elif self.agg == "hinton":
    ...
    elif self.agg == "random_select":
        random_indices = np.random.randint(0, len(f.estimators_), size=out.shape[1])
        return out[random_indices, range(out.shape[1])]
    elif self.agg == "median":
        return np.median(out, axis=0)
    elif self.agg == "mixture of experts":  # Vous pouvez spécifier l'indice de l'arbre fixe ici
        return out[self.fixed_tree_index]
    else:

...
class ForestDiffusionModel():
  def __init__(self, 
    ...
    assert fixed_tree_index < n_estimators, "The index of three must be inferior to the number of estimators"
    np.random.seed(seed)
    ...
    self.n_z = n_z
    self.agg = agg
    self.fixed_tree_index = fixed_tree_index
    ...
    if self.p_in_one:
      if self.n_jobs == 1:
        ...
        if self.n_batch > 0: # Data iterator, no need to duplicate, not make xt yet
          self.regr = Parallel(n_jobs=self.n_jobs)(delayed(self.train_iterator)(X1_splitted[j], X_covs_splitted[j], t=t_levels[i], dim=None) for i in tqdm(range(n_steps)) for j in self.y_uniques)
        else:
        ...
        current_i = 0
        for i in tqdm(range(n_steps),desc="replacing"):
          for j in range(len(self.y_uniques)): 
      ...
      if self.n_jobs == 1:
        ...
        if self.n_batch > 0: # Data iterator, no need to duplicate, not make xt yet
          self.regr = Parallel(n_jobs=self.n_jobs)(delayed(self.train_iterator)(X1_splitted[j], X_covs_splitted[j], t=t_levels[i], dim=k) for i in tqdm(range(n_steps)) for j in self.y_uniques for k in range(self.c))
        else:
          self.regr = Parallel(n_jobs=self.n_jobs)( # using all cpus
                  delayed(self.train_parallel)(
                    X_train.reshape(self.n_t, self.b*self.duplicate_K, self.c_all)[i][self.mask_y[j], :], 
  ...
  def train_parallel(self, X_train, y_train):
    ...
    if len(y_train.shape) == 1:
      ...
      #print("Energy of true noise:", np.mean(np.abs(y_train)), np.median(np.abs(y_train)))
  ...
  def predict_n(self, f, X_test):

    out = np.array([tree.predict(X_test) for tree in f.estimators_])
    
    if self.agg is None or self.agg == "none":
        return f.predict(X_test)
    elif self.agg == "arithmetic":
        return np.mean(out, axis=0)
    elif self.agg == "geometric":
        offset = abs(np.min(out)) + 1e-9
        return np.exp(np.mean(np.log(out + offset), axis=0)) - offset
    elif self.agg == "max_arg_values":
        max_indices = np.argmax(np.abs(out), axis=0)  # (1503, 6), indices des max en abs
        rows, cols = np.meshgrid(np.arange(out.shape[1]), np.arange(out.shape[2]), indexing="ij")
        return out[max_indices, rows, cols] 
  ...
  def predict_over_c(self, X, i, j, k, dmat, expand=False, X_covs=None):
    ...
    if k is None:
      return self.predict_n(self.regr[j][i],X_used)
    elif expand:
      return np.expand_dims(self.predict_n(self.regr[j][i][k],X_used), axis=1) # [b, 1]
    else:
      return elf.predict_n(self.regr[j][i][k],X_used)

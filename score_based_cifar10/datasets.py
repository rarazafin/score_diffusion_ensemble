...
def get_dataset(config, uniform_dequantization=False, evaluation=False, exclude_labels=""):
  ...
  def create_dataset(dataset_builder, split):
    ...
    if isinstance(dataset_builder, tfds.core.DatasetBuilder):
      ...
      ds = dataset_builder.as_dataset(
        split=split, shuffle_files= not evaluation, read_config=read_config)
    else:
      ds = dataset_builder.with_options(dataset_options)
    ds = ds.repeat(count=num_epochs)
    if not evaluation: ds = ds.shuffle(shuffle_buffer_size)
    if exclude_labels:
      exclude_labels_tf = tf.constant(list(map(int, exclude_labels.split(","))), dtype=tf.int64)
      ds = ds.filter(lambda d: tf.math.reduce_all(tf.math.not_equal(d['label'], exclude_labels_tf)))
